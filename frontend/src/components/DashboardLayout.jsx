...
const DashboardLayout = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex overflow-hidden">
      {isMobileOpen && (
        <div className="fixed inset-0 bg-slate-900/50 z-40 lg:hidden" onClick={() => setIsMobileOpen(false)} />
      )}
      ...
      >
        <div className="h-16 flex items-center justify-between px-4 border-b border-slate-200/50">
          ...
          <button className="lg:hidden text-slate-500 hover:text-slate-800" onClick={() => setIsMobileOpen(false)}>
            <X className="w-6 h-6" />
          </button>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        <header className="h-16 glass z-30 flex items-center justify-between px-4 sm:px-6 border-b border-slate-200/50 sticky top-0">
          <div className="flex items-center gap-4">
            <button className="lg:hidden text-slate-500 hover:text-slate-800 focus:outline-none" onClick={() => setIsMobileOpen(true)}>
